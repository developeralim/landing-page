...
// ------------------------------- AOS Animation
  ...
  // ------------------------------- template tabs
    ...
    tabs.each((index,tab) => {
      $(tab).on('click',function(e){
        ...
        for( var i = 0;i < tabs.length;i++ ){
          $(tabs[i]).removeClass('active-template');
        }
        $(this).addClass('active-template');

        var id = $(this).attr('id');

        $('.tab-fane').each((findex,fane) => {
          if ( id !== 'all-templates' ) {
            $(fane).removeClass('tab-fane-active');
          } else {
            $(fane).addClass('tab-fane-active');
          }
        })

        $(`div[data-category=${ $(this).attr('id') }]`).each(( i,t ) => {
          $(t).addClass('tab-fane-active');
        });
        toggleTemplateContainer();
      });
    });
    ...
    function toggleTemplateContainer(){
      var templateContainer = $('.ai-template-blocks-toggle')

      var rect = templateContainer[0].getBoundingClientRect();

      if ( rect.height > 1000 ) {
        $(templateContainer).addClass('hide-template-ui');
      } else {
        $(templateContainer).removeClass('hide-template-ui');
      }
    }
    ...
    $('.show-more').on('click',function(e){
      e.preventDefault();
      $('.ai-template-blocks-toggle').toggleClass('toggle-template')
    })
})(jQuery);